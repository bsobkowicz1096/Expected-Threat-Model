...
def main(args):
    ...
    with mlflow.start_run(run_name=args.run_name):
        ...
        # ====================================================================
        # STAGE 3: TRAIN ISOTONIC REGRESSION CALIBRATOR
        # ====================================================================
        
        print(f"\n{'='*80}")
        print("TRAINING ISOTONIC REGRESSION CALIBRATOR")
        print(f"{'='*80}")
        print("Computing calibrated xT on validation set...")
        
        # We already have val_xTs and val_labels from evaluate_xT
        # Extract them from metrics computation
        val_xTs_list = []
        val_labels_list = []
        
        print(f"Re-computing xT on validation set for calibration...")
        for i in tqdm(range(len(val_dataset)), desc="Computing xT"):
            sample = val_dataset[i]
            start_seq = {
                'input_types': sample['input_types'],
                'input_positions': sample['input_positions'],
                'input_start_mask': sample['input_start_mask']
            }
            
            xT = calculate_xT_montecarlo(model, start_seq, type_vocab, args.n_rollouts, max_steps=10, device=device)
            val_xTs_list.append(xT)
            
            label = (sample['target_types'] == type_vocab['GOAL']).any().item()
            val_labels_list.append(label)
        
        val_xTs_array = np.array(val_xTs_list)
        val_labels_array = np.array(val_labels_list)
        
        # Save uncalibrated predictions
        print(f"\nSaving uncalibrated validation predictions...")
        pd.DataFrame({
            'xT_uncalibrated': val_xTs_array,
            'label': val_labels_array  
        }).to_csv('val_predictions_100rollouts.csv', index=False)
        print("✅ Saved to val_predictions_100rollouts.csv")
        
        # Train Isotonic Regression
        print(f"\nTraining Isotonic Regression...")
        from sklearn.isotonic import IsotonicRegression
        isotonic = IsotonicRegression(out_of_bounds='clip')
        isotonic.fit(val_xTs_array, val_labels_array)
        print("✅ Isotonic Regression fitted!")
        
        # Evaluate calibrated
        val_xTs_calibrated = isotonic.predict(val_xTs_array)
        
        print(f"\nCalibration Results:")
        print(f"  Uncalibrated Brier: {brier_score_loss(val_labels_array, val_xTs_array):.5f}")
        print(f"  Calibrated Brier:   {brier_score_loss(val_labels_array, val_xTs_calibrated):.5f}")
        ...
        ece_uncal = calculate_ece(val_xTs_array, val_labels_array)
        ece_cal = calculate_ece(val_xTs_calibrated, val_labels_array)
        
        print(f"  Uncalibrated ECE:   {ece_uncal:.5f}")
        print(f"  Calibrated ECE:     {ece_cal:.5f}")
        print(f"  ECE Improvement:    {(1 - ece_cal/(ece_uncal+1e-10))*100:.1f}%")
        
        # Save Isotonic calibrator
        print(f"\nSaving Isotonic calibrator...")
        with open('isotonic_calibrator.pkl', 'wb') as f:
            pickle.dump(isotonic, f)
        print("✅ Saved to isotonic_calibrator.pkl")
        
        # Log to MLflow
        mlflow.log_metric("brier_uncalibrated", brier_score_loss(val_labels_array, val_xTs_array))
        mlflow.log_metric("brier_calibrated", brier_score_loss(val_labels_array, val_xTs_calibrated))
        mlflow.log_metric("ece_uncalibrated", ece_uncal)
        mlflow.log_metric("ece_calibrated", ece_cal)
        
        mlflow.log_artifact('isotonic_calibrator.pkl')
        mlflow.log_artifact('val_predictions_100rollouts.csv')
        ...
        print(f"  - isotonic_calibrator.pkl")
        print(f"  - val_predictions_100rollouts.csv")
        print("\n✅ Training complete!")
